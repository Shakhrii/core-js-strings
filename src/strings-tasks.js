...
function isPalindrome(str) {
  const strWithoutPunct = str.replace(
    /[\s/!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~]/g,
    ''
  );
  const reversedStr = [...strWithoutPunct].reverse().join('');
  return strWithoutPunct.toLowerCase() === reversedStr.toLowerCase();
}
...
function reverseWords(str) {
  ...
  wordsArray.forEach((word) => {
    const el = word.split('').reverse().join('');
...
function invertCase(str) {
  ...
  wordsArray.forEach((word) => {
    ...
    [...word].forEach((c) => {
      if (c === c.toLowerCase()) {
        casedWord.push(c.toUpperCase());
      } else {
        casedWord.push(c.toLowerCase());
      }
    });
    casedArray.push(casedWord.join(''));
  });
